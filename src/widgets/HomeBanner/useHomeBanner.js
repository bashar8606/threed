...
export const useHomeBanner = ({ style }) => {
  ...
  useLayoutEffect(() => {
    const ctx = gsap.context((self) => {
      ...
      const globe = self.selector(`.${style.shape}`)


      gsap.set(fade, { y: 100, autoAlpha: 0 })
      gsap.set(globe, { rotate: -30, scale: 0.6, autoAlpha: 0 })

      const tl = gsap.timeline({ repeat: 0, paused: true });
      tl.to(fade, { y: 0, autoRound: false,stagger: 0.1,  autoAlpha: 1, ease: Expo.easeOut, duration: 1.5 }, 0)
      tl.to(globe, { scale: 1, rotate: 0, autoAlpha: 1, ease: Expo.easeOut, duration: 2 }, 0.5)
